...
pair<int, int> forward_reg[16];                                                  // Forwarding Registers file
...
void forward(int value, int tag)
{ // Forwarding Function
    dec_to_hex(value);
    forward_reg[tag] = {value, 1};
}
...
public:
    ...
    { // Get Register Value
        ...
        else if (forward_reg[tag].second == 1)      //If register is forwarded
        {
            is_stalled = 0;
            R = forward_reg[tag].first;
            hex_to_dec(R);
        }
...
{ // Read Input
    ...
    for (int i = 0; i < size; i++)
    {
        if (!input)
            break;
        getline(input, ins);                // Read instruction from file
        arr[i] = stoi(ins, 0, 16);          // Convert to integer and store it in the array
    }
    input.close();
}
...
{ // Main
    ...
    { // Main Loop
        ...
        for (int i = 0; i < n; i++)
        {
            Instruction ins = IBuf.front();     // Get instruction from front of queue
            IBuf.pop();
            stall += ins.process();             // Process instruction
            if (ins.stage < 5)                  // If instruction is not complete
                IBuf.push(ins);
        }
        if (!can_fetch)                         
        { // If there is a control stall
            num_stall++;
            num_con_st++;
        }
        else if (stall)
